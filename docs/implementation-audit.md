...
| Roads | Generator creates one fixed urban plan: full east-west and north-south arterials cross wrapped map seams, while one rectangular collector loop adds four simple corners. Result has five clear four-way intersections, long straight approaches, and no stair-step bends or 2×2 asphalt blocks. Renderer builds one shared-vertex asphalt surface and uses compact eight-segment quarter-circle joins with an 18%-tile curb radius only at actual corners/intersections. Road terrain queries reuse same rounded footprint, so visible asphalt retains road handling. One geometry is instanced across nine repeating maps. Road lamps choose adjacent meadow positions. | World test verifies exact 63-cell plan, seed stability, full connectivity, both arterials, collector loop, five intersections, four corners, and zero asphalt blocks. Road-surface tests verify compact rounded joins reject old broad shoulders while retaining matching terrain classification and wrapped seam joins. Headless render confirms regular city blocks and small curb arcs. |
...
- Vitest: 4 files, 26 tests, all passing.
