...
| Ambient traffic | [`traffic.ts`](../src/game/traffic.ts) seeds up to ten road-following placeholders by default, cycling compact cars, bikes, vans, SUVs, and trucks. `maxTrafficVehicles` exposes a bounded scene parameter (hard cap 24). [`traffic-view.ts`](../src/game/traffic-view.ts) uses shared low-poly geometry and places each vehicle on the nearest wrapped map copy. | Traffic tests verify count cap, kind variety, road placement, deterministic movement, and zero-vehicle mode. Headless canvas capture shows moving truck/car placeholders on asphalt. |
...
| Roads | Seeded urban grammar selects collector-loop, parallel-grid, or staggered-block family. Every family starts with straight east-west and north-south arterials across wrapped map seams, then adds four balanced secondary arterials for extra blocks and intersections. Candidate bands retry when they would create 2×2 asphalt, so roads stay orthogonal and readable without staircase noise. Same seed reproduces same plan; “New map” can change both family and dimensions. Renderer uses supplied [`RoadTexture2.jpg`](../src/assets/roads/RoadTexture2.jpg) as actual asphalt map with an unlit material that preserves its neutral-gray color, one shared road geometry instanced across nine repeating maps, and compact eight-segment quarter-circle joins with an 18%-tile curb radius. Road terrain queries reuse same rounded footprint. | World test exercises one seed per grammar family and verifies three unique yet reproducible layouts, full connectivity, central arterials, at least twelve readable junctions, no more than four intentional corners, zero 2×2 asphalt blocks, and road counts between four and eight grid widths. Road-surface tests verify compact rounded joins, terrain classification, and wrapped seams. Production build emits supplied 68,560-byte asphalt asset; headless render confirms neutral-gray textured asphalt. |
