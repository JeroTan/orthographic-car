...
| Porsche vehicle model | [`porsche-model.ts`](../src/game/porsche-model.ts) loads supplied Porsche 911 GT2 geometry and textures only after main scene starts. [`build-porsche-model.mjs`](../scripts/build-porsche-model.mjs) merges fifteen body parts and quantizes source OBJ attributes into a 0.86 MB runtime binary, down from 2.98 MB; four wheel meshes remain separate for front steering and wheel spin. Existing chassis lean/stretch, rear-drive slip, trails, smoke, collision, and fallback car remain active. BMP source textures become compact WebP runtime assets. | Production typecheck/build succeeds. Software-WebGL capture shows textured red Porsche body, glass, lights, and four aligned wheels in world. |
