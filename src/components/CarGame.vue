...
function releaseControls(): void {
	for (const control of Object.keys(controls) as Control[]) controls[control] = false;
}
	...
	<main class="game-shell">
		...
		<header class="topbar">
			...
			<div class="world-panel">
				...
				<button type="button" @click="generateNewWorld">
					<span aria-hidden="true">↻</span><span class="button-label">New map</span>
				</button>
			</div>
		</header>
		...
		<section class="controls-panel" aria-label="Driving controls">
			...
			<div class="control-grid">
				...
				>
					<kbd>{{ button.key }}</kbd><span>{{ button.arrow }}</span>
				</button>
			</div>
		</section>

		<p class="world-note"><span>18 × 18</span> tiles · wraps north, south, east &amp; west</p>
...
.controls-panel {
	...
	box-shadow: 0 16px 40px rgb(55 71 58 / 13%), inset 0 1px 0 #fff;
}
...
@media (max-width: 760px) {
	...
	.brand-panel { padding: 0.8rem 0.9rem; }
	h1 { font-size: 1.65rem; }
	.world-panel { gap: 0.55rem; padding-left: 0.75rem; }
...
@media (prefers-reduced-motion: reduce) {
	.world-panel button { transition: none; }
}
</style>
